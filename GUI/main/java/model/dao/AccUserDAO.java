...
public class AccUserDAO extends DBconnect {
    // Đăng nhập mà không sử dụng mã hóa mật khẩu
    ...
    public List<AccUser> getAllUsers() throws SQLException {
        dbConnect.connect(); // Thiết lập kết nối
        connection = dbConnect.getConnection(); // Lấy kết nối từ DBconnect
        List<AccUser> userList = new ArrayList<>();
        String sql = "SELECT * FROM AccUser";  // Truy vấn SQL lấy tất cả người dùng

        // Thực hiện truy vấn và lấy kết quả
        try (PreparedStatement stmt = connection.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {

            // Duyệt qua các dòng kết quả và tạo đối tượng AccUser từ mỗi dòng
            while (rs.next()) {
                ...
                // Tạo đối tượng AccUser và thêm vào danh sách
